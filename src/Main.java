...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < allHands.length; i++) {
            int rankInc = 1;
            for (int j = 0; j < allHands.length; j++) {
                if (i != j && allHands[i].getWildType() > allHands[j].getWildType()) {
                    rankInc++;
                } else if (i != j && allHands[i].getWildType() == allHands[j].getWildType()) {
                    Boolean done = false;
                    String[] d = allHands[i].getCards();
                    String[] e = allHands[j].getCards();
                    for (int k = 0; k < d.length && !done; k++){
                        if (!d[k].equals(e[k])){
                            if (Deck.wildCardStrength(d[k]) > Deck.wildCardStrength(e[k])){
                                rankInc++;
                                done = true;
                            }
                            else {
            ...
            allHands[i].setWildRank(rankInc);
        }
        for (Deck hand : allHands) {
            wildBidTotal += (hand.getBidVal() * hand.getWildRank());
        }
        ...
        System.out.println("Total Bid Value With Jacks Wild: " + wildBidTotal);
    }
}